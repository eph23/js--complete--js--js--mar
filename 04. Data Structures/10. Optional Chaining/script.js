...
for (const day of weekDays) {
  ...
  console.log(
    `On ${day} we ${open !== undefined ? `open at ${open}` : 'are closed'}`,
  );
}

console.log(restaurant.order?.(1, 0) ?? 'Method dose not exists');
console.log(restaurant.orderRisotto?.(1, 0) ?? 'Method dose not exists');
